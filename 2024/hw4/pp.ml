...
module type KMINUS_PP = sig
  val pp : exp -> unit
end

module Kminus_PP : KMINUS_PP = struct
  let q x = [ "\"" ^ x ^ "\"" ]
  let pfx = "  "
  ...
  let rec qs xs =
    match xs with [] -> [] | [ hd ] -> q hd | hd :: tl -> comma (q hd) @ qs tl
  ...
  and pe e =
    ...
    | LETF (f, xs, e1, e2) ->
        ps "LETF" [ q f; ps "ARGS" [ qs xs ]; pe e1; pe e2 ]
    | CALLV (f, es) -> ps "CALLV" [ q f; ps "ARGS" (List.map pe es) ]
    | CALLR (f, ys) -> ps "CALLR" [ q f; ps "ARGS" [ qs ys ] ]
